...
const MobileApp = ({children}) => {
  ...
  useEffect(()=>{
    if(searchParams.get('page') === "nearby"){
      setOverFlow(true)
      return
    }
    setOverFlow(false)
  ...
  return (
    <motion.div 
      ...
      animate={keyboardOpen?{height:`${viewportHeight}px`}:{height:"100vh"}} className='fixed w-screen left-0 top-0 right-0 bottom-0' id="wrap">
